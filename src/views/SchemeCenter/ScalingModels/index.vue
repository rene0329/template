...
<template>
  <el-row :gutter="20">
    ...
    <el-col span="24" style="margin-top: 20px">
      ...
      -->
      <!-- 伸缩模型列表 -->
      <el-col v-for="info in ScalingModelInfo" :span="8" style="margin-top: 10px">
        <el-card shadow="always" class="ColForList List">
          <div slot="header" class="clearfix">
            <span>{{ info.scaling_model_name }}</span>
          </div>
          <div class="content" style="display: flex;margin-bottom: 10px;">
            <div class="imgs">
              <img :src="info.scaling_model_pictrue" alt="暂无图片">
            </div>
            <div class="descriptions">
              <p>{{ info.scaling_model_desc }}</p>
            </div>
          </div>
...
<script>
import {getScalingModelInfo} from '@/api/ScalingModel'
export default {
  data() {
    return {
      ScalingModelInfo: []
  ...
  methods: {
    fetchData() {
      getScalingModelInfo().then(res => {
        console.log(res)
        this.ScalingModelInfo = res.result
      })
    },
    // 同步各个卡片高度
    SetColSize() {
