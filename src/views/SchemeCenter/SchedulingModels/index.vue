...
<template>
  <el-row :gutter="20">
    ...
    <el-col span="24" style="margin-top: 20px">
      ...
      -->
      <!-- 调度模型列表 -->
      <el-col v-for="info in SchedulingModelInfo" :span="8" style="margin-top: 10px">
        <el-card shadow="always" class="ColForList List">
          <div slot="header" class="clearfix">
            <span>{{ info.sche_model_name }}</span>
          </div>
          <div class="content" style="display: flex;margin-bottom: 10px;">
            <div class="imgs">
              <img :src="info.sche_model_pictrue" alt="暂无图片">
            </div>
            <div class="descriptions">
              <p>{{ info.sche_model_desc }}</p>
            </div>
          </div>
...
<script>
import {getSchedulingModelInfo} from '@/api/SchedulingModel'
...
export default {
  ...
  methods: {
    fetchData() {
      getSchedulingModelInfo().then(res => {
        console.log(res)
        this.SchedulingModelInfo = res.result
      })
    },
    // 同步各个卡片高度
    SetColSize() {
