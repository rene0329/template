  ...
  <div class="scheduling-logs-page">
    <section class="content-card">
      <div class="page-heading">
        <div>
          <h2>调度日志</h2>
          <p>查看外部数据调度接口提交的计划及各数据分配项的执行结果。</p>
        </div>
      ...
      <el-table v-loading="loading" :data="plans" row-key="planId" empty-text="暂无外部数据调度记录">
      ...
      <div v-loading="detailLoading" class="detail-content">
        ...
        <template v-if="detail">
          ...
          <el-table :data="detail.assignments" row-key="assignmentId" size="small" empty-text="暂无数据分配记录">
            ...
            <el-table-column prop="datasetId" label="数据集 ID" width="100" />
            <el-table-column prop="replicaId" label="副本 ID" width="95" />
            <el-table-column prop="sourceNodeId" label="源节点 ID" width="100" />
            <el-table-column prop="targetNodeId" label="目标节点 ID" width="110" />
...
const actionLabels = { USE_IN_PLACE: '原位使用', COPY_AND_USE: '复制后使用', MOVE_AND_USE: '迁移后使用', REMOTE_READ: '远程读取' }
...
export default {
  ...
  methods: {
    ...
    async loadDetail() {
      ...
      this.detailError = ''
      try {
        const detail = await fetchSchedulingPlan(this.selectedPlanId)
