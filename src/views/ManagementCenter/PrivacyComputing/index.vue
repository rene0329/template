      ...
      <el-tabs v-model="activeTab" class="workspace-card" @tab-click="onTabClick">
        ...
        <el-tab-pane v-if="isDataOwner" label="发起计算" name="create">
          <el-form label-position="top" class="job-form">
            ...
            <div class="section-heading spaced"><div><h3>选择参与数据</h3><span>每个槽位选择一个不同业务域的数据集；各域的域用户同意后才会开始计算</span></div></div>
            <el-row :gutter="14">
              <el-col v-for="input in jobForm.inputs" :key="input.slotId" :xs="24" :lg="8">
                <div class="slot-card">
                  ...
                  <el-form-item label="数据集" required>
                    <el-select v-model="input.datasetId" filterable style="width:100%" placeholder="按业务域选择" @change="syncInputDataset(input)">
                      <el-option-group v-for="group in datasetGroups" :key="group.key" :label="group.label"><el-option v-for="dataset in group.datasets" :key="dataset.datasetId" :label="`${dataset.name || dataset.datasetCode} · ${dataset.version}`" :value="dataset.datasetId"><span>{{ dataset.name || dataset.datasetCode }}</span></el-option></el-option-group>
                    </el-select>
                  </el-form-item>
                  <div v-if="input.datasetId" class="snapshot"><div><span>业务域</span>{{ input.domainName }}</div><div><span>版本</span>{{ input.datasetVersion }}</div><div><span>摘要</span><code>{{ input.displaySha256 || '后端创建时校验' }}</code></div></div>
            ...
            <div class="form-actions"><el-button :loading="preflightLoading" :disabled="!canPreflight" @click="runPreflight">预检</el-button><el-button type="primary" :loading="creating" :disabled="!canCreate" @click="createJob">提交并等待各域审批</el-button><span v-if="preflight && preflight.specDigest" class="muted mono">{{ preflight.specDigest }}</span></div>
          </el-form>
        </el-tab-pane>
...
export default {
  ...
  computed: {
    ...
    pageDescription() { return { capabilities: '查看协议、安全档位、执行引擎和已知能力边界。', create: '选择模板及多个数据，系统向每个数据所在业务域发起授权审批。', approvals: '核对本域数据、字段用途、协议和泄露说明后作出决定。', jobs: '跟踪审批、运行状态、结果权限和证据。' }[this.activeTab] },
    ...
    datasetGroups() {
      const groups = {}
      // A participant's domain is where its dataset is stored; a dataset in no domain or in
      // several is rejected by the server (PARTICIPANT_DOMAIN_MISSING / _AMBIGUOUS), so only
      // datasets located in exactly one domain are offered.
      this.datasets.filter(item => this.datasetDomainId(item) != null).forEach(item => { const key = String(this.datasetDomainId(item)); if (!groups[key]) groups[key] = { key, label: this.datasetDomainName(item), datasets: [] }; groups[key].datasets.push(item) })
  ...
  methods: {
    ...
    datasetDomainId(dataset) { const ids = listOf(dataset.domainIds); return ids.length === 1 ? ids[0] : null }, datasetDomainName(dataset) { const names = listOf(dataset.domainNames); return names.length === 1 ? names[0] : '未归属域' },
    ...
    async createJob() { if (!this.canCreate) return; this.creating = true; try { const created = await createPrivacyJob(this.buildJobSpec(), privacyRequestId()); this.$message.success('任务已创建，正在等待各域审批'); this.activeTab = 'jobs'; await this.loadJobs(); const job = created.job || created; if (job.jobId) await this.openJob(job) } catch (error) { this.formError = `任务创建失败：${error.message}` } finally { this.creating = false } },
    ...
    jobOf(item) { return item.job || item }, jobIdOf(item) { return this.jobOf(item).jobId }, initiatorName(job) { return job.initiatorDisplayName || job.initiatorUsername || (typeof job.initiator === 'string' ? job.initiator : job.initiator && (job.initiator.displayName || job.initiator.username)) || '—' }, approvalDatasetName(item) { const direct = item.datasetName || (item.inputSnapshot && item.inputSnapshot.datasetName); if (direct) return direct; const participants = listOf(this.jobOf(item).participants); const domain = this.$store.getters.domain; const owned = domain && participants.find(input => String(input.ownerDomainId) === String(domain.id)); const datasetId = item.datasetId || (item.inputSnapshot && item.inputSnapshot.datasetId) || (owned && owned.datasetId); return `数据集 #${datasetId || '—'}` },
